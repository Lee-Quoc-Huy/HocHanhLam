...
export function MobileTabBar() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] px-3 lg:hidden pointer-events-none">
      <div className="pointer-events-auto mx-auto flex h-16 max-w-md items-center justify-around rounded-[1.75rem] border border-white/15 bg-background/80 p-1.5 shadow-[0_8px_32px_-4px_rgba(0,0,0,0.5)] backdrop-blur-2xl transition-all duration-300 ring-1 ring-white/10">
        {TAB_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              {isActive && (
                <span
                  aria-hidden
                  className="absolute inset-0 rounded-2xl bg-gradient-to-b from-emerald-500/25 via-emerald-500/10 to-transparent shadow-[0_0_20px_-2px_rgba(16,185,129,0.6)] ring-1 ring-emerald-400/40 animate-in fade-in zoom-in-95 duration-300"
                />
              )}
              <Icon
                className={cn(
                  ...
                  isActive
                    ? "scale-110 text-emerald-500 drop-shadow-[0_0_8px_rgba(16,185,129,0.7)]"
              ...
              <span
                className={cn(
                  ...
                  isActive ? "text-emerald-500 font-bold" : "text-muted-foreground/80"
                )}
              >
                {item.title}
              </span>
